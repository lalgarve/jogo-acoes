...
| Onde vive a lógica de consulta/agrupamento | `CompetitionViewService` novo, em `competition/`, injetando `ParticipationRepository`/`CompetitionRepository` — não reaproveita `CompetitionService` (hoje só criação/timing de convite, escopo de administrador) nem `EntryRequestService` (escopo de pedido de entrada). `CompetitionsController` ganha os três métodos novos (`listPublicCompetitions`, `listMyCompetitions`, `getCompetitionDetail` — nomes de `operationId`). | resolvida | Separar leitura (`CompetitionViewService`) de escrita (`CompetitionService`) evita crescer uma classe com responsabilidades muito diferentes; `CompetitionsController` continua sendo o único controller do recurso `/competitions*`. |
| `CompetitionsController` implementa qual(is) interface(s) gerada(s) | `listPublicCompetitions` ficou com a tag `competitions` (mesma de `createCompetition`/`decideInviteEmailTiming`), então cai em `CompetitionsApi` como as outras duas já existentes; `listMyCompetitions`/`getCompetitionDetail` ganharam uma tag nova, `my-competitions` (agrupamento próprio no Swagger UI), que o gerador OpenAPI-generator materializa como uma interface própria, `MyCompetitionsApi`. `CompetitionsController` passa a `implements CompetitionsApi, MyCompetitionsApi` — duas interfaces, uma classe só, como já decidido acima. | resolvida | Achado ao gerar o contrato (T003): tag decide qual interface Java o operationId cai, não o nome do path. Preferi uma tag nova pra separar visualmente "minhas competições" de "administração de competições" no Swagger UI, em vez de forçar as duas nas mesmas 3 operações de `CompetitionsApi` já existentes. |
...
- `CompetitionsController.java` (modificado) — passa a `implements CompetitionsApi,
  MyCompetitionsApi`; ganha `listPublicCompetitions` (na primeira), `listMyCompetitions`/
  `getCompetitionDetail` (na segunda).
