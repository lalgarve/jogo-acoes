...
| ~~T003~~ | Contrato: adicionar os schemas `CompetitionSummary`/`CompetitionDetail`/`AccessLevel`/`MyCompetitions`, os três caminhos novos (`GET /competitions/public`, `GET /competitions/mine`, `GET /competitions/{competitionId}`) e a extensão `x-roles` em toda operação de `docs/openapi.yaml` (todas as 15, novas e já existentes) — formaliza exatamente o que T001/T002 já esperavam, incluindo `canConfirmEntry`. Achados: (a) `listPublicCompetitions` tinha ficado com a tag `entry-requests` (herdada do placeholder antigo) — trocada pra `competitions`, senão caía numa interface Java diferente de `createCompetition`, contrariando o plano de tudo ficar em `CompetitionsController` (ver `plan.md`, decisão nova); `listMyCompetitions`/`getCompetitionDetail` ganharam tag própria (`my-competitions`), virando a interface `MyCompetitionsApi` — `CompetitionsController` agora implementa duas interfaces geradas, não uma; (b) reconfirmado T001/T002 depois do contrato: `browse_public_competitions` (jogador logado) mudou de `404` pra `501` (rota já reconhecida pelo `CompetitionsController` existente, ainda não implementada) — os sem sessão continuam `401` (falta T010); `view_my_competitions` continua todo em `404` (nenhum bean implementa `MyCompetitionsApi` ainda, falta T009) | T001, T002 | | #50 |
...
| ~~T006~~ | Criar `CompetitionAccessResolver` (função pura: `resolve(competition, participation, isAdministrator)` → `Optional<AccessLevel>`, vazio = `DENIED`), em `competition/` | T003 | [P] | #50 |
| ~~T007~~ | Teste dedicado de `CompetitionAccessResolver` cobrindo toda a matriz: participante em competição aberta (`READ_WRITE`); participou de competição encerrada (`READ`); convidado/pediu mas não confirmou (`READ`); administrador sem participar (`READ`); sem nenhuma relação, não administrador (vazio) — sem subir o contexto Spring; 5 testes, verde | T006 | [P] | #50 |
...
| ~~T009~~ | Implementar as três operações novas em `CompetitionsController` (gerado a partir de T003), delegando pra `CompetitionViewService`. Achado: `implements CompetitionsApi, MyCompetitionsApi` colide no método default `getRequest()` (as duas interfaces declaram o mesmo) — resolvido com um override explícito | T008 | | #50 |
| ~~T010~~ | Atualizar `SecurityConfig`: `/competitions/public` entra no `permitAll` já existente (troca o antigo `GET /competitions`); `/competitions/mine` e `GET /competitions/{competitionId}` não precisaram de regra nova — já caem em `.anyRequest().authenticated()` (qualquer papel autenticado — a checagem fina de acesso é do `CompetitionAccessResolver`, não da rota) | T009 | | #50 |
| ~~T011~~ | Rodar a suíte completa: **108 testes passando, 0 falhas** — os 4 cenários de `browse_public_competitions.feature` e os 10 de `view_my_competitions.feature` (incluindo o de "sem relação", agora `404` pelo motivo certo) todos verdes, mais todos os cenários já existentes sem alteração de texto Gherkin. Os 3 erros restantes da execução completa (`redirect_after_login.feature`) são da spec 05-005, ainda não implementada — fora de escopo aqui | T009, T010 | | #50 |
