...
|---|---|---|---|---|
| T001 | Criar `LinkDto` (record: `Long userId`, `String email`, `Map<String,String> extra`) em `{base}.link` | — | [P] | — |
| T002 | Criar a entidade JPA `LinkRecord` (`id`, `token`, `serviceKey`, `userId`, `email`, `extraJson`, `expiresAt`, `usedAt`) e seu repositório | — | [P] | — |
| T003 | Criar `LinkOutcome` (resultado: autenticado com dado de redirecionamento, ou "registro pendente") | — | [P] | — |
| T004 | Criar a interface `LinkHandler` (`String key()`; `LinkOutcome consume(LinkDto dto)`; `default LinkOutcome complete(LinkDto dto, Map<String,String> extra)` lançando `UnsupportedOperationException`) | — | [P] | — |
...
| T006 | Criar `LinkService` (`create(serviceKey, dto)`, `consume(token)`, `complete(token, extra)`), usando Jackson só para serializar/desserializar `extra` de/para `extraJson` — `userId`/`email` vão direto para as colunas | T001, T002, T005 | | — |
| T007 | Criar `LinkController` (endpoint de consumo recebendo só o token; endpoint de conclusão de registro pendente) | T006 | | — |
| T008 | Criar `LoginLinkHandler` em `{base}.login` (`key() = "login"`, só `consume()`), cobrindo o caso de login avulso hoje em `LoginService.consumeLoginLink` | T004 | [P] | — |
| T009 | Criar `CompetitionLinkHandler` em `{base}.competition` (`key() = "competition-entry"`; `consume()` autentica se `dto.userId() != null`, senão devolve "pendente"; `complete()` cria `User`, vincula `Participation` via `extra["participationId"]`, status `IN_COMPETITION`) | T004 | [P] | — |
...
| T013 | Migrar as três call sites que hoje criam `LoginLink` ligado a uma `Participation` diretamente (`EntryRequestService.requestEntry`, `PlayerManagementService.sendInviteEmail`, `CompetitionService.decideInviteEmailTiming`) para chamar `LinkService.create("competition-entry", dto)` | T006, T009 | | — |
| T014 | Migrar `LoginService.requestLoginLink` (login avulso) para chamar `LinkService.create("login", dto)` | T006, T008 | | — |
