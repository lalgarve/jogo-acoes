...
|---|---|---|---|---|
| ~~T001~~ | Adicionar dependência `com.tngtech.archunit:archunit-junit5` (escopo `test`, versão mais recente estável) ao `app/pom.xml` — usada a `1.5.0` (última estável no Maven Central em 2026-08-04) | — | [P] | #53 |
...
| ~~T003~~ | Escrever `common/ArchitectureTest.java` (regra ArchUnit: toda classe `@RestController` está num pacote-base que também contém uma classe implementando `SecurityConfigContributor`) e rodar — confirmado vermelho: exatamente as 4 violações esperadas (`CompetitionsController`, `EntryRequestsController`, `PlayersController` em `competition`; `LoginController` em `login`, nenhum com `SecurityConfigContributor` ainda) | T001, T002 | | #53 |
| ~~T004~~ | Escrever `common/RouteOwnershipTest.java` (via `RequestMappingHandlerMapping`: nenhuma rota mapeia `/`; cada primeiro segmento de path pertence a um único módulo) e rodar — confirmado verde de cara (2 testes, nenhuma mudança de rota ainda), como esperado — trava de regressão, não ciclo vermelho/verde | — | [P] | #53 |
| ~~T005~~ | Criar `login/LoginSecurityConfigContributor.java` (`@Component`), movendo para lá os matchers `permitAll` de `/login-requests`, `/login-links/**` que hoje estão em `SecurityConfig` — ainda não conectado ao `SecurityFilterChain` (isso é T007), mas já confirmado que o `ArchitectureTest` caiu de 4 pra 3 violações (só `login` coberto) | T002 | | #53 |
| ~~T006~~ | Criar `competition/CompetitionSecurityConfigContributor.java` (`@Component`), movendo para lá os matchers de `/competitions*` (públicos e administrativos) que hoje estão em `SecurityConfig` — ainda não conectado ao `SecurityFilterChain` (T007); com os dois contributors já existindo, o `ArchitectureTest` já reporta 0 violações (achado antecipado da T008, reconfirmada formalmente depois da T007) | T002 | | #53 |
...
| ~~T008~~ | Rodar `ArchitectureTest` de novo — confirmado verde | T003, T007 | | #53 |
| ~~T009~~ | Rodar a suíte completa (`mvn test`) — confirmado: **122 testes, 0 falhas, 0 erros** (119 já existentes + `ArchitectureTest` + `RouteOwnershipTest` ×2), nenhuma rota/regra de acesso mudou | T004, T008 | | #53 |
