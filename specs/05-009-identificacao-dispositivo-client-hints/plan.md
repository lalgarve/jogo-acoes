...
| Como servir Swagger UI a partir do `docs/openapi.yaml` estático | Nova dependência `springdoc-openapi-starter-webmvc-ui`, com `springdoc.swagger-ui.url` apontando pro arquivo estático (copiado/servido a partir de `src/main/resources/static/openapi.yaml`) e `springdoc.api-docs.enabled: false` (não escanear anotações pra gerar um segundo contrato). | resolvida | O contrato já é a fonte de verdade (API-first, spec 05-007 valida isso continuamente) — gerar um segundo contrato via anotação criaria duas fontes conflitantes. |
