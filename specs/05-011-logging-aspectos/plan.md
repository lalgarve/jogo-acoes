...
| Como suprimir em produção | `application-production.yml` ganha `logging.level.dev.leilaalgarve.jogoacoes.common.logging: OFF` — os três aspectos logam sob esse pacote (logger nomeado pela própria classe, que cai dentro dele). Nenhuma checagem de perfil dentro do código do aspecto. | resolvida | Usa o mecanismo de configuração que o Spring Boot já dá de graça por perfil — não inventa um `@ConditionalOnProperty`/checagem de `Environment` dentro de cada aspecto. |
| Guarda de performance | Cada aspecto checa `logger.isInfoEnabled()` antes de montar a mensagem (serializar argumentos é trabalho evitável quando o nível está desligado). | resolvida | Barato de fazer, evita gastar CPU serializando em produção só pra descartar a `String` em seguida. |
...
- `app/src/main/resources/application-production.yml` (modificado) — nível de log suprimido.
