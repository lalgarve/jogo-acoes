...
| ~~T003~~ | Criar `ReturnToValidator` ("só caminho relativo do próprio site": começa com uma única `/`, sem `//` nem `\` logo depois — o segundo bloqueia um bypass conhecido de checagem ingênua de same-origin) — função pura, em `login/`. Achado: o schema OpenAPI original (`nullable: true` num campo opcional) gerava `JsonNullable<String>` em vez de `String` simples — removido `nullable: true` de `returnTo` em `docs/openapi.yaml`, já que não precisamos distinguir "ausente" de "null" | T002 | [P] | #51 |
| ~~T004~~ | Teste dedicado do validador de T003: caminhos relativos válidos aceitos; URLs absolutas (`https://evil.com`), protocol-relative (`//evil.com`) e o bypass por barra invertida rejeitados, sem lançar erro (retorna vazio/`Optional`) — 7 testes, verde | T003 | [P] | #51 |
| ~~T005~~ | Atualizar `LoginController.requestLoginLink` para ler `returnTo` do corpo, validar com `ReturnToValidator`, e gravar em `LinkPayload.extra["returnTo"]` ao chamar `linkService.create("login", ...)` | T003 | | #51 |
| ~~T006~~ | Atualizar `LoginLinkHandler` para receber o `LinkPayload` completo em `redirectFor` (hoje só recebia o `userId`) e checar `payload.extra().get("returnTo")` antes de calcular o padrão por papel (`/admin` ou `/competitions/mine`, agora caminhos completos) — usado tanto em `consume` quanto em `alreadyAuthenticated` | T005 | | #51 |
...
| ~~T008~~ | Simplificar `LoginController.toLoginResult`: repassar o caminho direto (sem mapear enum, sem montar `competitionId` à parte) — feita antecipadamente na T002, mecanicamente forçada pela quebra de compilação (ver nota lá) | T006, T007 | | #51 |
| ~~T009~~ | Atualizar os testes existentes afetados pela mudança de contrato: `LoginLinkHandlerTest` (3 asserções + 1 teste novo cobrindo o `returnTo` honrado), `CompetitionLinkHandlerTest` (3 asserções), `LoginSteps` (`assertRedirectsToCompetitionPage`, os dois steps de destino admin/lista) — todos trocando os símbolos antigos (`admin-page`/`competitions-list`/`competition-page`+`competitionId`) pelos caminhos novos | T008 | | #51 |
| ~~T010~~ | Rodar a suíte completa: **119 testes, 0 falhas, 0 erros** — os 3 cenários de `redirect_after_login.feature` verdes (incluindo o de substituição de destino), mais todos os cenários/testes já existentes, sem nenhuma alteração de texto Gherkin | T009 | | #51 |
