...
> **Nota:** as iterações 4 e 6 foram inferidas a partir de um diagrama de arquitetura
...
## Iteração 5 — Redução de bounce no SES
...
reputação do remetente) e é pré-requisito prático para sair do modo sandbox do SES (Iteração
4, decisão em aberto).
...
- Duas checagens síncronas no momento em que o e-mail é coletado (convite de administrador,
  ...
  falha rejeita o cadastro imediatamente, sem criar `Participation`/`LoginLink` pra um
  e-mail que não vai receber nada.
  - **Registro MX do domínio**: consulta DNS confirma que o domínio está configurado para
    receber e-mail. Pega domínio inexistente ou digitado errado; não pega uma caixa
    inexistente num domínio válido (ex. `usuario-que-nao-existe@gmail.com` passa).
  - **Domínio descartável/temporário**: contra uma lista de bloqueio mantida localmente
    (Postgres), carregada a partir de
    [`disposable-email-domains`](https://github.com/disposable-email-domains/disposable-email-domains)
    (arquivo `disposable_email_blocklist.conf`, em ordem alfabética).
- Processo agendado (Spring `@Scheduled`, dentro do `app/` — sem infraestrutura nova)
  rodando 1x/dia: busca o arquivo do GitHub, calcula o diff contra a versão anterior (a
  lista já vem ordenada, o que facilita), aplica só as entradas adicionadas/removidas na
  tabela local.

**Fora de escopo:** verificação de existência real da caixa postal (*probe* SMTP `VRFY` ou
similar) — não confiável e mal-visto por provedores de e-mail; MX + lista de descartáveis é
o filtro de baixo custo. Bounce por caixa cheia, servidor temporariamente fora do ar, ou
mailbox que existiu e foi desativada continuam não detectáveis por aqui — isso é o que a
observabilidade da Iteração 6 mede depois do fato, não algo que dá pra prevenir na entrada.

## Iteração 6 — Deploy, observabilidade e hardening
...
## Iteração 7 — BDD da competição (negociação de ações) e novo DER
...
- Sem Ibovespa — comparação de portfólio fica só entre jogadores (ver Iteração 12).

**Fora de escopo:** implementação (Iteração 8).

## Iteração 8 — Persistência e regras de negociação

**Objetivo:** implementar o DER da Iteração 7 e as regras de negócio por trás dos cenários
BDD.
...
## Iteração 9 — Coleta de cotações (Brapi)
...
## Iteração 10 — Telas dos fluxos já especificados (login, competição, jogadores)
...
## Iteração 11 — Telas de negociação e portfólio
...
- Mesmo critério de aceite da Iteração 10: navegação testada com leitor de tela.

## Iteração 12 — Gráficos (preço, portfólio, comparação entre jogadores)
...
## Iteração 13 — Acessibilidade de gráficos: pesquisa e modelagem — descrição por pontos importantes
...
## Iteração 14 — Acessibilidade de gráficos: implementação — descrição por pontos importantes

**Objetivo:** implementar o modo de descrição textual definido na Iteração 13.
...
## Iteração 15 — Acessibilidade de gráficos: pesquisa e modelagem — sonorização
...
## Iteração 16 — Acessibilidade de gráficos: implementação — sonorização

**Objetivo:** implementar o modo de sonorização definido na Iteração 15.
