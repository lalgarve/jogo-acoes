...
erDiagram
    ...
    LOGIN_SESSION {
        ...
        string device_id
        string http_session_id "links to the real HTTP session, spec 05-010"
        datetime created_at
        datetime ended_at
    }
    LOG {
...
- **`LOGIN_SESSION.http_session_id`** (spec 05-010) liga o registro à sessão HTTP real (`spring_session.session_id`, Spring Session JDBC) que a autenticação de fato usa — gravado em `establish()` depois de `securityContextRepository.saveContext(...)`. É o que permite a gestão de sessões ativas (`GET`/`DELETE /sessions`) ter efeito real: revogar chama `SessionRepository.deleteById(httpSessionId)`, apagando a sessão de verdade, não só marcando `ended_at` no nosso próprio registro.
